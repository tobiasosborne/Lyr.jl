...
function convert_tinyvdb_internal1(node::TinyVDB.InternalNodeData, origin::Coord,
    ...
    # Sort children by linear index for popcount-based lookup
    sorted_children = sort(node.children, by=first)

    children = LeafNode{Float32}[
        convert_tinyvdb_leaf(child_data::TinyVDB.LeafNodeData, child_origin_internal1(origin, Int(linear_idx)))
        for (linear_idx, child_data) in sorted_children
    ]

    n_tiles = count_on(value_mask)
    tiles = Tile{Float32}[Tile{Float32}(background, true) for _ in 1:n_tiles]

    InternalNode1{Float32}(origin, child_mask, value_mask, children, tiles)
end
...
function convert_tinyvdb_internal2(node::TinyVDB.InternalNodeData, origin::Coord,
    ...
    children = InternalNode1{Float32}[
        convert_tinyvdb_internal1(child_data::TinyVDB.InternalNodeData, child_origin_internal2(origin, Int(linear_idx)), background)
        for (linear_idx, child_data) in sorted_children
    ]
    ...
    InternalNode2{Float32}(origin, child_mask, value_mask, children, tiles)
end
